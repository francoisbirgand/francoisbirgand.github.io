...
First, the code to make a list of all directories down a directory tree is:
...
<br/>
That is pretty much all I thought I could share. The final exact code I used is below:
<br/>

```{r echo=TRUE, eval= FALSE}
setwd<-c("/Users/francoisbirgand/Google Drive/Echantillonnage/Methods/Data")
dirlist<-list.dirs(recursive = TRUE)      # recursive = TRUE means that all dir in the tree will appear
...
filelist<-list.files(recursive = TRUE)    # recursive = TRUE means that all files in the tree will appear
