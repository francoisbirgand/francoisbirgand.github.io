...
<div class = "col-md-4">
<br/><br/><br/><br/>
<img style="align: center; width: 100%;" src="images/CLMD_surf.jpg">
</div>
</div>
...
for (type in types){
  tmp_tutblog <- tutblog[tutblog$type == type,]
  years <- unlist(unique(tmp_tutblog$year))[order(unlist(unique(tmp_tutblog$year)),decreasing = TRUE)]
  cat("\n") 
  cat("###",type,"\n")
  for (year in years){
    ...
    for (i in 1:nrow(tmp_tutblog)){
      cat("    + ",paste0("[",tmp_tutblog[i,]$name,"](",tmp_tutblog[i,]$reference,")"),paste0(" -- *updated ",tmp_tutblog[i,]$date,"*"), sep = "")
      cat("\n")
