...
My goal was to transform all dates into `yyyy-mm-dd hh:mm:ss`, which is the only robust format I could find.
<br/><br/>

### Applying *`strsplit()`* onto a vector to extract dates and times

`strsplit()` stands for "string split" and this function will split at a given pattern, e.g., spaces, ":", "/", etc. In the exammple below, the result of the function is a list:
<br/>
...
Now, I am really looking for a vector with the date and a vector with the time, and playing with lists is not that easy.  The way to do this is to use the `do.call()` function just like this:
<br/><br/>
...
Notice that in the variable `temptime`, the first value is not a time variable.  This is because in the original data, which was probably from Excel, the time values at midnight, which should be "0:00" are omitted. This must be taken care of later.
...
The only additional trick here is the automatic check that there is a "time" component to the initial date, that if there are no seconds, they are automatically added, and that all years are automatically transformed into "yyyy".
<br/><br/>
...
datime<-do.call( rbind , strsplit( as.character(dates) , " " ) )
      ...
      if (dim(datime)[2]==2){
        tempdate<-datime[,1]
        temptime<-datime[,2]
        temptime[grep("/",temptime)]="0:0"     # used to flag the times replaced by a "d/m/y" date
        ...
        if (dim(temptime)[2]==3){sec = temptime[,3]}else{sec="00"}
        T= paste0(hr,":",min,":",sec)
